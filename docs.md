...
- `sample_notes.md`: 部門、システム、モデルの関係を文章で説明するMarkdown
- `sample_faq.txt`: 単純なQA検索を試しやすいText
- `sample_project_overview.pdf`: PDF読込確認用の概要資料

Markdown、Text、PDFを混在させることで、通常RAGでは断片的に拾われやすく、GraphRAGでは関係をまとめて説明しやすい構成にしています。
